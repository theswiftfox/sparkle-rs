...
impl<T> CBuffer<T> {
    pub fn create(data: T, context: *mut dx11_1::ID3D11DeviceContext1, device: *mut dx11_1::ID3D11Device1) -> Result<CBuffer<T>, DxError> {
    ...
    pub fn update(&self) -> Result<(), DxError>{
        let mut mapped : dx11::D3D11_MAPPED_SUBRESOURCE = Default::default();
        let res = unsafe {
            (*self.context).Map(self.gpu_buffer as *mut _, 0, dx11::D3D11_MAP_WRITE_DISCARD, 0, &mut mapped as *mut _)
        };
        if res < winapi::shared::winerror::S_OK {
            return Err(DxError::new("CBuffer update failed", DxErrorType::ResourceUpdate));
        }
        unsafe { std::ptr::copy_nonoverlapping(&self.data as *const T, mapped.pData as *mut T, 1) };

        Ok(())
    }

    pub fn buffer_ptr(&self) -> *const *mut dx11::ID3D11Buffer {
        &self.gpu_buffer as *const *mut _
    }
    ...
    fn create_gpu_resource(&mut self, device: *mut dx11_1::ID3D11Device1) -> Result<(), DxError> {
        ...
        let res = unsafe { 
            (*device).CreateBuffer(
                &desc,
                &initial_data,
                &mut self.gpu_buffer as *mut *mut _,
            )
        };
        if res < winapi::shared::winerror::S_OK {
            return Err(DxError::new("CBuffer create failed", DxErrorType::ResourceCreation));
        }
        Ok(())
    }
}